...
import { sanitizeHtml, escapeJS } from "../security";
...
export const colors = {
  primary: '${escapeJS(ds.colors.primary)}',
  secondary: '${escapeJS(ds.colors.secondary)}',
  accent: '${escapeJS(ds.colors.accent)}',
  background: '${escapeJS(ds.colors.background)}',
  surface: '${escapeJS(ds.colors.surface)}',
  text: '${escapeJS(ds.colors.text)}',
  textSecondary: '${escapeJS(ds.colors.textSecondary)}',
  success: '${escapeJS(ds.colors.success)}',
  warning: '${escapeJS(ds.colors.warning)}',
  error: '${escapeJS(ds.colors.error)}',
};

export const spacing = {
${Object.entries(ds.spacing.scale)
            .map(([key, value]) => `  '${escapeJS(key)}': '${escapeJS(String(value))}',`)
            .join("\n")}
};

export const typography = {
  fontFamily: {
    heading: '${escapeJS(ds.typography.fontFamily.heading)}',
    body: '${escapeJS(ds.typography.fontFamily.body)}',
    mono: '${escapeJS(ds.typography.fontFamily.mono)}',
  },
  sizes: {
${Object.entries(ds.typography.sizes)
            .map(([key, value]) => `    '${escapeJS(key)}': '${escapeJS(String(value))}',`)
 ...
 */
const DesignTokens = () => (
  <div style={{ padding: '2rem', fontFamily: '${escapeJS(ds.typography.fontFamily.body)}' }}>
    <h1 style={{ fontFamily: '${escapeJS(ds.typography.fontFamily.heading)}', fontSize: '${escapeJS(ds.typography.sizes['3xl'])}' }}>
      Design System Tokens
    </h1>
    
    <section style={{ marginTop: '2rem' }}>
      <h2 style={{ fontSize: '${escapeJS(ds.typography.sizes.xl)}', borderBottom: '1px solid #eee', paddingBottom: '0.5rem' }}>Colors</h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '1rem', marginTop: '1rem' }}>
        ${Object.entries(ds.colors)
            .map(([name, value]) => {
                ...
                return `
        <div key="${escapeJS(name)}">
          <div style={{ backgroundColor: '${escapeJS(String(colorValue))}', height: '60px', borderRadius: '${escapeJS(ds.borderRadius.md)}', border: '1px solid #ddd' }} />
          <p style={{ fontSize: '12px', marginTop: '4px' }}><strong>${sanitizeHtml(name)}</strong></p>
          <p style={{ fontSize: '10px', color: '#666' }}>${sanitizeHtml(String(colorValue))}</p>
        </div>`;
            })
            .join("")}
      </div>
    ...
    <section style={{ marginTop: '3rem' }}>
      <h2 style={{ fontSize: '${escapeJS(ds.typography.sizes.xl)}', borderBottom: '1px solid #eee', paddingBottom: '0.5rem' }}>Typography</h2>
      <div style={{ marginTop: '1rem' }}>
        ${Object.entries(ds.typography.sizes)
            .map(([size, value]) => `
        <p style={{ fontSize: '${escapeJS(String(value))}', margin: '1rem 0' }}>The quick brown fox jumps over the lazy dog (${sanitizeHtml(size)}: ${sanitizeHtml(String(value))})</p>`).join("")}
      </div>
    </section>
  </div>
);
...
export function generateTokensReference(ds: GeneratedDesignSystem, _tokens: DesignToken[]): string {
    return `# Design Tokens Reference: ${sanitizeHtml(ds.name)}
...
${Object.entries(ds.colors).map(([name, val]) => {
        ...
        return `| **${sanitizeHtml(name)}** | Color | \`${sanitizeHtml(value)}\` | Base design system color |`;
    }).join("\n")}

## 🔠 Typography

- **Heading Font:** ${sanitizeHtml(ds.typography.fontFamily.heading)}
- **Body Font:** ${sanitizeHtml(ds.typography.fontFamily.body)}
- **Mono Font:** ${sanitizeHtml(ds.typography.fontFamily.mono)}

### Scale
| Name | Value |
|------|-------|
${Object.entries(ds.typography.sizes).map(([name, val]) => `| ${sanitizeHtml(name)} | ${sanitizeHtml(String(val))} |`).join("\n")}
...
| Scale | Value |
|-------|-------|
${Object.entries(ds.spacing.scale).map(([name, val]) => `| ${sanitizeHtml(name)} | ${sanitizeHtml(String(val))} |`).join("\n")}

## 🪄 Effects
### Border Radius
| Name | Value |
|------|-------|
${Object.entries(ds.borderRadius).map(([name, val]) => `| ${sanitizeHtml(name)} | ${sanitizeHtml(String(val))} |`).join("\n")}

### Shadows
| Name | Value |
|------|-------|
${Object.entries(ds.shadows).map(([name, val]) => `| ${sanitizeHtml(name)} | ${sanitizeHtml(String(val))} |`).join("\n")}
`;
}
...
<head>
    ...
    <title>${sanitizeHtml(ds.name)} - Brand Guidelines</title>
    <style>
        body { font-family: '${escapeJS(ds.typography.fontFamily.body)}', sans-serif; line-height: 1.6; color: #333; max-width: 800px; margin: 0 auto; padding: 40px; }
        h1, h2, h3 { font-family: '${escapeJS(ds.typography.fontFamily.heading)}', sans-serif; color: #111; }
        ...
        .token { font-family: '${escapeJS(ds.typography.fontFamily.mono)}', monospace; background: #f7f7f7; padding: 2px 4px; border-radius: 4px; }
    </style>
</head>
<body>
    <h1>${sanitizeHtml(ds.name)} Brand Guidelines</h1>
    <p>Generated by DesignForge</p>

    <h2>Primary Colors</h2>
    <div class="swatch-grid">
        <div class="swatch">
            <div class="swatch-color" style="background-color: ${escapeJS(ds.colors.primary)}"></div>
            <div class="swatch-info">
                <strong>Primary</strong><br>
                <span class="token">${sanitizeHtml(ds.colors.primary)}</span>
            </div>
        </div>
        <div class="swatch">
            <div class="swatch-color" style="background-color: ${escapeJS(ds.colors.secondary)}"></div>
            <div class="swatch-info">
                <strong>Secondary</strong><br>
                <span class="token">${sanitizeHtml(ds.colors.secondary)}</span>
            </div>
        </div>
        <div class="swatch">
            <div class="swatch-color" style="background-color: ${escapeJS(ds.colors.accent)}"></div>
            <div class="swatch-info">
                <strong>Accent</strong><br>
                <span class="token">${sanitizeHtml(ds.colors.accent)}</span>
            </div>
        </div>
    </div>

    <h2>Typography</h2>
    <div style="font-family: '${escapeJS(ds.typography.fontFamily.heading)}'; font-size: ${escapeJS(ds.typography.sizes['4xl'])};">${sanitizeHtml(ds.name)} Heading Font: ${sanitizeHtml(ds.typography.fontFamily.heading)}</div>
    <div style="font-family: '${escapeJS(ds.typography.fontFamily.body)}'; font-size: ${escapeJS(ds.typography.sizes.xl)}; margin-top: 20px;">Body Font: ${sanitizeHtml(ds.typography.fontFamily.body)}</div>
    ...
    <div style="display: flex; gap: 10px; align-items: flex-end;">
        ${Object.entries(ds.spacing.scale).map(([name, val]) => `
            <div style="text-align: center;">
                <div style="background: ${escapeJS(ds.colors.primary)}; width: ${escapeJS(String(val))}; height: ${escapeJS(String(val))}; margin: 0 auto 5px;"></div>
                <div style="font-size: 10px;">${sanitizeHtml(name)}</div>
            </div>
        `).join('')}
    </div>
</body>
...
export function generateStyleguideMD(ds: GeneratedDesignSystem): string {
