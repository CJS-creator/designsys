...
export function previewDesignSystemPdf(
  ds: GeneratedDesignSystem,
  versionInfoOrOptions?: VersionInfo | DesignSystemPdfOptions,
): { url: string; blob: Blob; filename: string; pdf: jsPDF } {
  // Back-compat: previously the second arg was just VersionInfo.
  const opts: DesignSystemPdfOptions =
    versionInfoOrOptions && ("orientation" in versionInfoOrOptions || "sections" in versionInfoOrOptions || "coverThumbnail" in versionInfoOrOptions || "previewOnly" in versionInfoOrOptions || "filename" in versionInfoOrOptions)
      ? { ...(versionInfoOrOptions as DesignSystemPdfOptions), previewOnly: true }
      : { previewOnly: true, versionInfo: versionInfoOrOptions as VersionInfo | undefined };
  const pdf = exportDesignSystemToPdf(ds, opts);
