...
export function injectDesignSystemVariables(system: GeneratedDesignSystem) {
    ...
    if (system.colors.primary) root.style.setProperty('--primary', hexToHSL(typeof system.colors.primary === 'string' ? system.colors.primary : (system.colors.primary as { hover?: string }).hover || '#000'));
